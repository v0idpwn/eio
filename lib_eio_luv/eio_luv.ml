...
let to_eio_addr_info (addr_info:Luv.DNS.Addr_info.t) : Eio.Dns.Addr_Info.t =
  Obj.magic addr_info

let dns = object
  inherit Eio.Dns.t

  method getaddrinfo ?family ?protocol  ?node ?service () =
    await_exn (fun loop _fiber -> Luv.DNS.getaddrinfo ~loop ?family ?protocol ?node ?service ())
    |> List.map to_eio_addr_info
end
